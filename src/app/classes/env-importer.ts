...
export class EnvironmentImporter extends BaseImporter {
  ...
  public createEnvItem(env: any, id: number): Item {
    ...
    forkJoin({
      ...
      tap(({ atmo, body, wind }) => {
        // Patch atmo data.
        if (!!atmo) this.patchAtmoForm(envItem, atmo as AtmoFormValues);
        // Patch body data
        if (!!body) patchSubsectionForm(envItem, 'body', 'general', body);
        // Patch wind data.
        if (!!wind) this.patchWindForm(envItem, wind as WindFormValues);
      })
    ).subscribe();
  ...
  public getGravData(env: any): GravityFormValues {
    if (env.gravity) {
      ...
      return {} as GravityFormValues;
    }
  }

  public getAtmoData(env: any): Observable<AtmoFormValues | boolean> {
    if (env.atmosphere) {
  ...
  public getBodyData(env: any): Observable<BodyFormValues | boolean> {
    if (env.body) {
  ...
  public getWindData(env: any): Observable<WindFormValues | boolean> {
    if (env.wind) {
